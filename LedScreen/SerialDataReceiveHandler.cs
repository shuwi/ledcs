...
namespace LedScreen
{
    public class SerialDataReceiveHandler
    {
        ...
        private PictureBox inuserpictureBox;
        private PictureBox outuserpictureBox;
        ...
        public SerialDataReceiveHandler(SerialPort serialPort, Control control, int tag, ListBox lstBoxWorker, List<Image> images, PictureBox inuserpictureBox, PictureBox outuserpictureBox) : this(serialPort, control)
        {
            workerService = new WorkerService();
            this.Tag = tag;
            this.images = images;
            this.lstBoxWorker = lstBoxWorker;
            this.inuserpictureBox = inuserpictureBox;
            this.outuserpictureBox = outuserpictureBox;
        }
        ...
        {
            //int imgIndex = this.lstBoxWorker.Items.Count;
            ...
            lstBoxWorker.Items.Insert(0,userInfo);
            
            //lstBoxWorker.SelectedIndex = this.lstBoxWorker.Items.Count - 1;
            //lstBoxWorker.SelectedIndex = -1;
            if (type == 1)
                inuserpictureBox.BackgroundImage = new Bitmap(new MemoryStream(Convert.FromBase64String(base64)));
            else
                outuserpictureBox.BackgroundImage = new Bitmap(new MemoryStream(Convert.FromBase64String(base64)));
